...
class CPGNetwork(NeuralNetwork):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.d_high = d_high
        self.a_rate = a_rate
        self.a_rate = np.tile(a_rate, 2)  # (8,) -> (16,) for both sides
        self.offset_freq = offset_freq
        self.offset_freq = np.tile(offset_freq, 2)  # (8,) -> (16,) for both sides
        self.offset_amp = offset_amp
        self.offset_amp = np.tile(offset_amp, 2)  # (8,) -> (16,) for both sides
        self.G_freq = G_freq
        self.G_freq = np.tile(G_freq, 2)  # (8,) -> (16,) for both sides
        self.G_amp = G_amp
        self.G_amp = np.tile(G_amp, 2)  # (8,) -> (16,) for both sides
        self.PL = PL
        self.PL = np.tile(PL, 2)  # (8,) -> (16,) for both sides
        ...
        # ── Nominal frequencies & amplitudes (Eq. 7 & 8) ──────────────────────
        def _drive_val(G, offset, d, d_low, d_high):
            return np.where((d < d_low) | (d > d_high), 0.0,
                            G * (d - d_low) + offset)

        for side, drive in [(self.left_body_idx, drive_left),
                            (self.right_body_idx, drive_right)]:
            self.nominal_frequencies[side] = _drive_val(
                G_freq, offset_freq, drive, d_low, d_high)
            self.nominal_amplitudes[side] = _drive_val(
                G_amp, offset_amp, drive, d_low, d_high)

        # ── Coupling weights (Eq. 4) ───────────────────────────────────────────
        n = self.n_oscillators          # 2 * n_body_joints
        nj = self.n_body_joints         # oscillators per side

        for i in range(n):
            for j in range(n):
                same_side = (i // 2 == j // 2 % 2 == i % 2)  # same L/R parity
                i_side = i % 2   # 0 = left, 1 = right
                j_side = j % 2
                i_seg  = i // 2  # segment index
                j_seg  = j // 2
        ...
        # ── Phase biases (Eq. 5) ──────────────────────────────────────────────
        PB = float(np.sum(PL)) / nj   # uniform phase bias between successive joints
        for i in range(n):
            for j in range(n):
                i_side = i % 2
                j_side = j % 2
                ...
                if i_side == j_side:           # ipsilateral
                    if j_seg == i_seg - 1:     # upwards (tail→head)
                        self.phase_bias[i, j] = PB
                    elif j_seg == i_seg + 1:   # downwards (head→tail)
                        self.phase_bias[i, j] = -PB
                else:                          # contralateral
                    if i_seg == j_seg:
                        if i_side == 0:        # left → right
    ...
    def network_ode(self, _time, state, stretch_value):
        ...
        # Phase ODE (Eq. 3): θ̇i = 2πfi + Σj rj wij sin(θj − θi − ϕij)
        phase_diff = phases[np.newaxis, :] - phases[:, np.newaxis] - self.phase_bias
        coupling   = amplitudes[np.newaxis, :] * self.coupling_weights * np.sin(phase_diff)
        dstates[:self.n_oscillators] = \
        ...
        if self.w_ipsi is not None and np.all(abs(amplitudes) > 0.00001):
            ...
            dstates[self.n_oscillators:2*self.n_oscillators] += stretch_feedback*np.cos(all_stretch_vals)
            dstates[:self.n_oscillators] -= stretch_feedback/amplitudes*np.sin(all_stretch_vals)
        return dstates
